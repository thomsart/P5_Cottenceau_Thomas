...
class Database():
    ...
    def fill_tables(self, food, nb_pages):
        ...
        """ We take from Openfoodfact Api the 'food' we want and put it in a list 
        of values in order to fill the table made for it in our database. 
        ...
        self.cursor.execute("""INSERT INTO category (name) VALUE('"""+str(food)+
            """') """)

        self.cursor.execute("""SELECT id FROM category WHERE name = '"""+str(food)+
            """' """)
        ...
        for i in range(nb_pages):
            ...
            data = response.json()        # we convert the type of the document 
                                          # in Json. we catch all the number of 
            key = data.get("products")    # differents products which are actually 
            number_of_product = len(key)  # Keys of a dict. now we count them in a int/type.
        ...
        for el in the_ids:
            self.cursor.execute("""INSERT INTO product_category (id_product, 
                id_category) VALUES('"""+str(el[0])+"""', '"""+str(id_cat)+"""') 
                """)
